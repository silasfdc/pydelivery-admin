...
interface Product { id: string; name: string; description: string|null; price: number; isAvailable: boolean; categoryId: string; category?: {name:string}; }
...
export default function ProductsPage() {
  ...
  const [form, setForm] = useState({name:'',description:'',price:0,categoryId:'',isAvailable:true});
  const fetchData = async () => { try { const [p,c] = await Promise.all([api.get('/products'),api.get('/categories')]); setProducts(p.data); setCategories(c.data); } catch(e){console.error(e);} finally{setLoading(false);} };
  useEffect(() => { fetchData(); }, []);
  const submit = async (e: React.FormEvent) => { e.preventDefault(); try { const d={...form,price:Number(form.price)}; if(editingId) await api.patch(`/products/${editingId}`,d); else await api.post('/products',d); setForm({name:'',description:'',price:0,categoryId:'',isAvailable:true}); setShowForm(false); setEditingId(null); await fetchData(); } catch(e){console.error(e);} };
  ...
  const del = async (id:string) => { try { await api.delete(`/products/${id}`); setDeletingId(null); await fetchData(); } catch(e){ console.error(e); setDeletingId(null); } };
  const toggle = async (p:Product) => { try{await api.patch(`/products/${p.id}`,{isAvailable:!p.isAvailable}); await fetchData();}catch(e){console.error(e);} };
  return (
    <DashboardLayout>
      <div className="space-y-5">
        ...
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] overflow-hidden"><table className="w-full text-sm"><thead><tr className="border-b border-[var(--border)] text-[var(--text-muted)]"><th className="text-left p-4 font-medium">Producto</th><th className="text-left p-4 font-medium">Categoría</th><th className="text-left p-4 font-medium">Precio</th><th className="text-left p-4 font-medium">Estado</th><th className="text-left p-4 font-medium">Acciones</th></tr></thead><tbody>
          {products.map(p=><tr key={p.id} className="border-b border-[var(--border)] hover:bg-[var(--bg-hover)]"><td className="p-4"><div className="font-medium">{p.name}</div>{p.description&&<div className="text-xs text-[var(--text-muted)]">{p.description}</div>}</td><td className="p-4 text-[var(--text-secondary)]">{p.category?.name||'—'}</td><td className="p-4 font-bold text-emerald-400">₲{fmt(p.price)}</td><td className="p-4"><button onClick={()=>toggle(p)} className={`px-2 py-1 rounded-lg text-xs font-medium ${p.isAvailable?'bg-emerald-500/20 text-emerald-300':'bg-red-500/20 text-red-300'}`}>{p.isAvailable?'✅ Activo':'❌ Inactivo'}</button></td><td className="p-4"><div className="flex gap-2">
            <button onClick={()=>edit(p)} className="px-2 py-1 rounded-lg text-xs bg-blue-600/20 text-blue-300">✏️</button>
            {deletingId===p.id ? (
              <div className="flex gap-1 items-center">
                <button onClick={()=>del(p.id)} className="px-2 py-1 rounded-lg text-xs bg-red-600 text-white font-medium animate-pulse">Sí</button>
                <button onClick={()=>setDeletingId(null)} className="px-2 py-1 rounded-lg text-xs bg-gray-600/30 text-gray-300">No</button>
              </div>
            ) : (
              <button onClick={()=>setDeletingId(p.id)} className="px-2 py-1 rounded-lg text-xs bg-red-600/20 text-red-300">🗑️</button>
            )}
          </div></td></tr>)}
